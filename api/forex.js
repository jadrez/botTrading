// Module-level candle cache — 60s TTL per symbol (warm Vercel instances)
const _candleCache = {};
const CANDLE_TTL = 60 * 1000;

async function fetchFromYahoo(from, to, wantCandles, limit) {
  const sym = `${from}${to}=X`;
  const r = await fetch(
    `https://query1.finance.yahoo.com/v8/finance/chart/${sym}?interval=1m&range=2d`,
  ...
  return { candles, rate: currentPrice || candles.at(-1)?.c, source: "yahoo" };
}

export default async function handler(req, res) {
  if (req.method !== "POST") return res.status(405).end();

  const { from = "EUR", to = "USD", candles: wantCandles = false, limit = 200 } = req.body || {};
  ...
  // Primary: Yahoo Finance — real 1-min OHLCV, no API key needed
  try {
    const data = await fetchFromYahoo(from, to, wantCandles, limit);
  ...
  // Fallback: ECB hourly (price only — no candles available here)
  try {
